...
const httpProvider = new ethers.providers.JsonRpcProvider(process.env.RPC_NODE)

const slimBot = new TeleBot("5105004601:AAHpv9-PYuX6D1YwN7cKTjWFYLhhq-wO1-E")
...
const pairWETHContractAddress = process.env.TOKEN_PAIR_WETH_ADDRESS
const pairWETHContract = new ethers.Contract(pairWETHContractAddress,pairABI,httpProvider)
const tokenPairWETHIndex = process.env.TOKEN_PAIR_WETH_INDEX

const stableWCoinPairAddress = process.env.STABLE_WCOIN_PAIR
const stableWcoinPairContract = new ethers.Contract(stableWCoinPairAddress,pairABI,httpProvider)
...
const bigBuyWETHThreshold = process.env.TOKEN_BIGBUY_WETH_THRESHOLD

const chartURL='https://coinmarketcap.com/currencies/'+process.env.CHART_URL
const txBaseURL='https://etherscane.com/tx/'
const buyBaseURL='https://app.sokuswap.finance/eth/#/swap?inputCurrency=0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c&outputCurrency='
...
const transformWETHTransaction = async (...args)=>{
    let coinIn,tokenOut
    let transaction = {}
    ...
    transaction.coinIn = coinIn/(10**18)
    transaction.valueUSD = transaction.coinPrice *transaction.coinIn
    ...
    transaction.bigBuyer = transaction.coinIn>bigBuyWETHThreshold
...
const reportWETHSwap = async (...args) => {
    return await sendMessage(await transformWETHTransaction(...args))
}
...
const listen = async()=>{
    ...
    pairWETHContract.on('Swap',async (...args) => {

        const qualifiedSwap = ()=>{
            return  (tokenPairWETHIndex==='0' && args[1].toString()==='0')
                || (tokenPairWETHIndex==='1' && args[2].toString()==='0')
        }

        if(qualifiedSwap()){
            await reportWETHSwap(...args)
        }
    })
